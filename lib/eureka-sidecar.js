...
module.exports = {
	...
	initialize: function(override_options) {
        ...
        this.config = SplunkCloud.DynamicConfig(this.options.cloud_stack);

        this.retrieve_eureka_url(function(err, eureka_url ) {
            if ( err ) {
                throw err;
            }
	...
	retrieve_eureka_url: function(callback) {
		var metadata = {
			"action": "retrieve_eureka_url",
			"status": "start"
		};
		winston.info("Retrieving Eureka URL", metadata);
		...
		this.config.get_eureka_url(function (err, eureka_url) {
			...
			metadata["table"] = this.config.table;
			metadata["eureka_url"] = eureka_url;
			winston.info("Retrieving Eureka URL", metadata);
			callback(null, this.options.eureka_url);
		}.bind(this));
	},
